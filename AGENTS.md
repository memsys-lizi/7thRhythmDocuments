...
| 阶段 5：事件与效果执行 | 进行中 | 已完成事件执行总览 API 页和事件到效果调度模块页，覆盖 `LevelEventType` 分层、`scnGame.ApplyEventsToFloors`、`scnGame.ApplyEvent`、`ffxPlusBase`、`scnGame.PrepVfx`、`scrVfxPlus.Update`、`runOnHit`、`runManually`、条件事件、输入事件和 hitbox 触发；下一步按事件族拆写相机滤镜、轨道地板、装饰对象、文本声音、输入粒子等对照页。 |
