...
| 阶段 1：核心骨架 | 已完成 | 已完成 `ADOBase`、`ADOClass`、`ADOStartup`、`scrController`、`scrConductor`、`scnGame`、`scnEditor`、`scrLevelMaker`、`scrFloor`，以及核心启动、运行时控制器、音频节拍、自定义关卡运行、路径地板和编辑器主入口模块页；下一步进入阶段 2：关卡数据模型。 |
