...
| 阶段 5：事件与效果执行 | 进行中 | 已完成事件执行总览、事件到效果调度、轨道与地板事件、相机滤镜与屏幕事件 API 页和模块页，覆盖 `LevelEventType` 分层、`scnGame.ApplyEventsToFloors`、`scnGame.ApplyEvent`、`ffxPlusBase`、`scnGame.PrepVfx`、`scrVfxPlus.Update`、`runOnHit`、`runManually`、条件事件、输入事件、hitbox 触发、轨道地板事件、`MoveCamera`、`Flash`、`SetFilter`、`SetFilterAdvanced`、`HallOfMirrors`、`ShakeScreen`、`Bloom`、`ScreenTile` 和 `ScreenScroll`；下一步拆写装饰对象、文本声音或输入粒子事件族。 |
