...
| 阶段 4：运行时游戏系统 | 进行中 | 已完成运行时输入与判定、控制器状态、暂停与练习流程、相机与 VFX 运行链路、结算与成绩保存、场景流转与加载跳转 API 页和模块页，覆盖 `RDInput`、`RDInputType`、普通键盘、异步键盘、`AsyncInputManager`、`AsyncInputUtils`、`scrController.UpdateInput`、`scrPlayer.Simulated_PlayerControl_Update`、`scrPlayer.Hit`、`scrPlanet.SwitchChosen`、`scrMisc.GetHitMargin`、输入限制、`scrFloor.LightUp`、`States`、`Start_Rewind`、`OnMusicScheduled`、`Countdown`、`Checkpoint`、`PlayerControl`、`TogglePauseGame`、`PauseMenu`、`SetPracticeMode`、`PracticeTimeline`、`FailAction`、`Fail2Action`、`Won`、`scrCamera`、`scrVfxPlus`、`ffxPlusBase`、`ffxCameraPlus`、`ffxSetFilterPlus`、`ffxSetFilterAdvancedPlus`、`ffxFlashPlus`、`ffxShakeScreenPlus`、`ffxBloomPlus`、`scrMarginTracker`、`scrMistakesManager`、`DetailedResults`、`EndscreenLanterns`、`scrPercentageComplete`、`scrFailBar`、`scrLoader`、`PortalTravelAction`、`EnterLevel`、`EnterWorld`、`LoadCustomLevel`、`LoadCustomWorld`、`QuitToMainMenu`、`ResetCustomLevel` 和 `scnGame.ResetScene`；下一步补更多运行时效果族和关卡脚本运行入口。 |
