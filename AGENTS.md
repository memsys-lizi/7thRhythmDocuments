...
| 阶段 4：运行时游戏系统 | 进行中 | 已完成运行时输入与判定 API 页和模块页，覆盖 `RDInput`、`RDInputType`、普通键盘、异步键盘、`AsyncInputManager`、`AsyncInputUtils`、`scrController.UpdateInput`、`scrPlayer.Simulated_PlayerControl_Update`、`scrPlayer.Hit`、`scrPlanet.SwitchChosen`、`scrMisc.GetHitMargin`、输入限制和 `scrFloor.LightUp`；下一步补控制器状态机细节、暂停/练习流程和相机/VFX 运行链路。 |
