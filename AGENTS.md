...
| 阶段 7：文件级覆盖与复核 | 进行中 | 已完成 ADOFAI [文件级覆盖清单](/api/review/source-coverage.md) 第一轮、[CameraFilterPack 文件族索引](/api/review/camera-filterpack-coverage.md) 和 [旧式 ffx 效果组件索引](/api/review/legacy-ffx-coverage.md)：已把 61 个 `CameraFilterPack_*` 后处理组件和 48 个剩余 `ffx*` 旧式效果组件全部列入索引；下一步处理剩余 `scr*` 组件。 |
