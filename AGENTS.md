...
| 阶段 2：关卡数据模型 | 进行中 | 已完成 `LevelData`、`LevelEvent` 和关卡数据模型模块页，覆盖 `.adofai` 文件读取、settings、actions、decorations、事件属性字典、编码解码与版本兼容主线；下一步补 `LevelEventInfo`、`PropertyInfo`、`Property` 和事件类型枚举。 |
