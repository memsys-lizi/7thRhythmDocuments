...
| 阶段 6：平台、存档、服务与 UI | 进行中 | 已完成 ADOFAI [全局状态、常量与存档](/api/platform/global-state-persistence.md) 和 [平台、存档与全局状态模块](/modules/platform-persistence-services.md)，覆盖 `GCS`、`GCNS`、`Persistence`、world metadata、general/custom prefs、官方世界进度、自定义关卡成绩、checkpoint progress、成就同步、云存档比较和延迟保存；下一步继续平台 helper、Steam、DLC 与服务初始化。 |
