...
| 文档 `.md` 文件总数 | 73 | PowerShell 递归统计 `adofai/docs`。 |
| 文件名或类型名已在文档中命中 | 463 | 用文档正文匹配 `.cs` 文件名或去扩展名后的类型名。 |
| 文件名或类型名尚未在文档中命中 | 759 | 其中包含第三方目录、滤镜库、示例脚本、小型 UI、旧式 `scr*` 和旧式 `ffx*` 类。 |
...
| `CameraFilterPack_*` | 61 | 相机滤镜库类；已在滤镜流程中说明接入，阶段 7 需要建立滤镜文件族索引。 |
| `ffx*` | 48 | 旧式效果组件和官方关卡效果；已覆盖 `ffxPlusBase` 与主要 `ffx*Plus`，剩余文件按旧式效果专题归类。 |
...
| 批次 | 文件族 | 目标页面 |
| --- | --- | --- |
| 7.1 | `CameraFilterPack_*`、滤镜 shader 驱动组件、`scrVfxPlus` 中滤镜字典使用点 | 相机滤镜文件族索引。 |
| 7.2 | 剩余 `ffx*`、旧式官方关卡效果、sprite tween、相机 tween、菜单效果 | 旧式效果与官方演出效果补充。 |
...
| 7.7 | 第三方目录归档表和全站链接复核 | 第三方依赖边界与全站复核页。 |
...
阶段 7 已开始。当前页面完成了第一轮文件级统计、目录分布、根目录未命中文件族和补齐批次规划。下一轮从 `CameraFilterPack_*` 文件族开始，因为这批文件数量集中，且和已经完成的相机滤镜事件、`scrVfxPlus`、`ffxSetFilterPlus`、`ffxSetFilterAdvancedPlus` 页面可以直接建立交叉链接。
