...
| 文档 `.md` 文件总数 | 78 | PowerShell 递归统计 `adofai/docs`。 |
| 文件名或类型名已在文档中命中 | 655 | 用文档正文匹配 `.cs` 文件名或去扩展名后的类型名。 |
| 文件名或类型名尚未在文档中命中 | 567 | 其中包含第三方目录、示例脚本、小型 UI 和剩余 `scr*` 类。 |
...
| 7.3 | 剩余 `scr*` 条件开关、按钮、菜单、文本、调试 HUD 和小型 UI | 进行中：第一批见 [scr UI、条件与文本辅助组件索引](/api/review/scr-ui-condition-coverage.md)，第二批见 [scr 动画、相机与 HUD 辅助组件索引](/api/review/scr-animation-hud-camera-coverage.md)。 |
...
阶段 7 已开始。当前页面完成了第一轮文件级统计、目录分布、根目录未命中文件族和补齐批次规划；`CameraFilterPack_*` 文件族已经由 [CameraFilterPack 文件族索引](/api/review/camera-filterpack-coverage.md) 收口。下一轮处理剩余 `ffx*` 旧式效果组件，因为它们与事件执行和官方关卡演出直接相连。
