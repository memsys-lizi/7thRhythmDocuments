...
| 阶段 4：运行时游戏系统 | 进行中 | 覆盖控制器状态机、输入、判定、轨道生成、地板、相机、音频、VFX、暂停、结算和场景流程 | 已完成 [运行时输入与判定](/api/runtime/input-judgement.md)、[运行时输入与判定链路](/modules/runtime-input-judgement.md)、[控制器状态、暂停与练习流程](/api/runtime/controller-pause-flow.md)、[控制器、暂停与练习流程](/modules/runtime-controller-pause.md)、[相机与 VFX 运行链路](/api/runtime/camera-vfx-chain.md) 和 [相机与 VFX 运行时](/modules/runtime-camera-vfx.md)，覆盖 `RDInput`、普通键盘、异步键盘、`AsyncInputManager`、`scrController.UpdateInput`、`scrPlayer.Simulated_PlayerControl_Update`、`scrPlayer.Hit`、`scrPlanet.SwitchChosen`、`scrMisc.GetHitMargin`、输入限制、`scrFloor.LightUp`、`States`、`Start_Rewind`、`OnMusicScheduled`、`Countdown`、`Checkpoint`、`PlayerControl`、`TogglePauseGame`、`PauseMenu`、`SetPracticeMode`、`PracticeTimeline`、`FailAction`、`Fail2Action`、`Won`、`scrCamera`、`scrVfxPlus`、`ffxPlusBase`、`ffxCameraPlus`、`ffxSetFilterPlus`、`ffxSetFilterAdvancedPlus`、`ffxFlashPlus`、`ffxShakeScreenPlus` 和 `ffxBloomPlus`；下一步补结算、场景流程辅助和更多运行时效果族 |
