...
| 阶段 6：平台、存档、服务与 UI | 进行中 | 覆盖 `Persistence`、`GCS`、`GCNS`、平台 helper、Steam、DLC、CLS、关卡选择、菜单、移动端 UI 和本地化 | 已完成 [全局状态、常量与存档](/api/platform/global-state-persistence.md)、[平台 Helper、DLC、Steam 与服务](/api/platform/platform-dlc-steam-services.md) 和 [平台、存档与全局状态模块](/modules/platform-persistence-services.md)，覆盖 `GCS`、`GCNS`、`Persistence`、world metadata、general/custom prefs、官方世界进度、自定义关卡成绩、checkpoint progress、成就同步、云存档比较、延迟保存、平台 helper、DLC manager、SteamIntegration、SteamWorkshop、GameServices 和 Analytics；下一步继续 CLS、关卡选择、菜单、移动端 UI 与本地化 |
