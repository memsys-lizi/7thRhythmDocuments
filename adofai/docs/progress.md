...
| 阶段 4：运行时游戏系统 | 进行中 | 覆盖控制器状态机、输入、判定、轨道生成、地板、相机、音频、VFX、暂停、结算和场景流程 | 已完成 [运行时输入与判定](/api/runtime/input-judgement.md)、[运行时输入与判定链路](/modules/runtime-input-judgement.md)、[控制器状态、暂停与练习流程](/api/runtime/controller-pause-flow.md)、[控制器、暂停与练习流程](/modules/runtime-controller-pause.md)、[相机与 VFX 运行链路](/api/runtime/camera-vfx-chain.md)、[相机与 VFX 运行时](/modules/runtime-camera-vfx.md)、[结算、成绩与进度保存](/api/runtime/results-save-flow.md) 和 [结算与成绩保存模块](/modules/runtime-results-save.md)，覆盖 `RDInput`、普通键盘、异步键盘、`AsyncInputManager`、输入判定、`States`、暂停、checkpoint、练习、胜利失败、`scrCamera`、`scrVfxPlus`、`ffxPlusBase`、相机与滤镜效果、`scrMarginTracker`、`scrMistakesManager`、`DetailedResults`、`EndscreenLanterns`、`scrPercentageComplete` 和 `scrFailBar`；下一步补场景流程辅助、加载跳转和更多运行时效果族 |
