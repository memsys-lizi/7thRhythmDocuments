...
| 阶段 4：运行时游戏系统 | 进行中 | 覆盖控制器状态机、输入、判定、轨道生成、地板、相机、音频、VFX、暂停、结算和场景流程 | 已完成 [运行时输入与判定](/api/runtime/input-judgement.md) 和 [运行时输入与判定链路](/modules/runtime-input-judgement.md)，覆盖 `RDInput`、`RDInputType`、普通键盘、异步键盘、`AsyncInputManager`、`AsyncInputUtils`、`scrController.UpdateInput`、`scrPlayer.Simulated_PlayerControl_Update`、`scrPlayer.Hit`、`scrPlanet.SwitchChosen`、`scrMisc.GetHitMargin`、输入限制和 `scrFloor.LightUp`；下一步补控制器状态机细节、暂停/练习流程和相机/VFX 运行链路 |
