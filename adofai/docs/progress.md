...
| 阶段 7：文件级覆盖与复核 | 进行中 | 建立 1222 个 `.cs` 文件覆盖清单，补齐未分类文件，复核链接、术语表、调用图和全站一致性 | 已完成 [文件级覆盖清单](/api/review/source-coverage.md) 第一轮和 [CameraFilterPack 文件族索引](/api/review/camera-filterpack-coverage.md)：统计 1222 个源码文件、75 个文档文件、529 个文件名或类型名命中和 693 个待补命中文件，并把 61 个 `CameraFilterPack_*` 后处理组件全部列入索引；下一步处理剩余 `ffx*` 旧式效果组件 |
