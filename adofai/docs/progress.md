...
| 阶段 2：关卡数据模型 | 进行中 | 深写 `LevelData`、`LevelEvent`、`LevelEventInfo`、`PropertyInfo`、`Property`、`LevelEventType`、`LevelEventCategory`、`LevelEventExecutionTime`、序列化与校验 | 已完成 [LevelData](/api/data-models/LevelData.md)、[LevelEvent](/api/data-models/LevelEvent.md) 和 [关卡数据模型](/modules/level-data-model.md)，已覆盖 `.adofai` 文件读取、settings、actions、decorations、事件属性字典、编码解码与版本兼容主线；下一步补 `LevelEventInfo`、`PropertyInfo`、`Property` 和事件类型枚举 |
